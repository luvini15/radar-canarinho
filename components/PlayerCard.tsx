...
import type { PlayerMedia, PlayerSummary } from "@/lib/types";
import { formatCompact, initials } from "@/lib/metrics";

export function PlayerCard({ player, compact=false }:{ player:PlayerSummary; compact?:boolean }) {
  const [media, setMedia] = useState<PlayerMedia | null>(null);

  useEffect(() => {
    let alive = true;
    fetch(`/api/player-media?name=${encodeURIComponent(player.nome)}`)
      .then(r => r.json())
      .then(data => { if (alive) setMedia(data); })
      .catch(() => { if (alive) setMedia(null); });
    return () => { alive = false; };
  }, [player.nome]);

  const foto = media?.fotoUrl || "";
  const escudo = media?.escudoUrl || "";
  const clube = media?.clubeApi || "Clube não identificado";
  const posicao = media?.posicaoApi || "Instagram";
  ...
  return (
    <article className="sticker-card relative overflow-hidden rounded-3xl p-4 shadow-md transition hover:-translate-y-1 hover:shadow-xl">
      <div className="flex items-start justify-between">
        <div>
          <span className="block text-xs font-black uppercase tracking-wider text-brasil-azul">Instagram</span>
      ...
      <p className="text-center text-xs font-semibold text-slate-600">@{player.username}</p>
      ...
      {!compact && (
        <div className="mt-2 flex items-center justify-center gap-2 text-center text-xs font-bold text-slate-700">
          {escudo ? (
            ...
            <span className="h-5 w-5 rounded-full bg-brasil-azul text-[9px] leading-5 text-white">CL</span>
          )}
      ...
      <div className="mt-4 rounded-2xl bg-white/60 p-3 text-center">
        <div className="font-display text-2xl text-brasil-azul">{formatCompact(player.seguidores)}</div>
        <div className="text-xs font-bold uppercase text-slate-600">seguidores</div>
      </div>
    </article>
  );
}